...
export function getAllClients(): Client[] {
  return getDb().prepare('SELECT * FROM clients ORDER BY created_at DESC').all() as Client[];
}
...
export function createClient(data: Partial<Client> & { adSoyad?: string }): number {
  const db = getDb();
  const alias = (data as any).adSoyad ?? data.alias ?? 'Anonim';
  const feeRaw = (data as any).seansUcreti ?? data.seans_ucreti;
  const result = db.prepare(
    `INSERT INTO clients (alias, age, gender, occupation, marital_status, referral_source, telefon, email, status, sunum_sorunu, hedefler, seans_ucreti, takip_sikligi, kisilik_tipi)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    alias,
    data.age ?? null,
  ...
  const fResult = db.prepare(`INSERT INTO formulations (client_id) VALUES (?)`).run(clientId);
  const formulationId = fResult.lastInsertRowid as number;
  db.prepare(`INSERT INTO flexibility_scores (formulation_id) VALUES (?)`).run(formulationId);

  return clientId;
}

export function updateClient(id: number, data: Partial<Client> & { adSoyad?: string; cinsiyet?: string; sunumSorunu?: string; exitReason?: string; seansUcreti?: number | null; takipSikligi?: string | null; kisilikTipi?: string | null }): void {
  ...
  values.push(id);
  getDb().prepare(`UPDATE clients SET ${updates.join(', ')} WHERE id = ?`).run(...values);
}
...
export function getFormulationByClient(clientId: number): Formulation | null {
  return (getDb().prepare('SELECT * FROM formulations WHERE client_id = ?').get(clientId) as Formulation) ?? null;
}

export function updateFormulation(id: number, data: Partial<Formulation>): void {
  const fields = [
  ...
  updates.push('updated_at = CURRENT_TIMESTAMP');
  values.push(id);
  getDb().prepare(`UPDATE formulations SET ${updates.join(', ')} WHERE id = ?`).run(...values);
}

// ─── Formulation Items ───────────────────────────────────────────────────────

export function getItemsByCategory(formulationId: number, category: string): FormulationItem[] {
  return getDb().prepare(
    'SELECT * FROM formulation_items WHERE formulation_id = ? AND category = ? ORDER BY order_index ASC, id ASC'
  ).all(formulationId, category) as FormulationItem[];
}

export function getItemsByFormulation(formulationId: number): FormulationItem[] {
  return getDb().prepare(
    'SELECT * FROM formulation_items WHERE formulation_id = ? ORDER BY category, order_index ASC, id ASC'
  ).all(formulationId) as FormulationItem[];
}

export function addItem(formulationId: number, category: string, content: string): number {
  const result = getDb().prepare(
    'INSERT INTO formulation_items (formulation_id, category, content) VALUES (?, ?, ?)'
  ).run(formulationId, category, content);
  return result.lastInsertRowid as number;
}

export function deleteItem(id: number): void {
  getDb().prepare('DELETE FROM formulation_items WHERE id = ?').run(id);
}

// ─── Flexibility Scores ──────────────────────────────────────────────────────

export function getScores(formulationId: number): FlexibilityScores | null {
  return (getDb().prepare('SELECT * FROM flexibility_scores WHERE formulation_id = ?').get(formulationId) as FlexibilityScores) ?? null;
}

export function updateScores(formulationId: number, data: Partial<FlexibilityScores>): void {
  ...
  if (updates.length === 0) return;
  updates.push('updated_at = CURRENT_TIMESTAMP');
  values.push(formulationId);
  getDb().prepare(`UPDATE flexibility_scores SET ${updates.join(', ')} WHERE formulation_id = ?`).run(...values);
}

// ─── Seanslar ────────────────────────────────────────────────────────────────

export function getAllSeanslar(): (Seans & { adSoyad: string })[] {
  return getDb().prepare(
    `SELECT s.*, c.alias as adSoyad FROM seanslar s
     LEFT JOIN clients c ON c.id = s.client_id
     ORDER BY s.tarih DESC`
  ).all() as (Seans & { adSoyad: string })[];
}

export function createSeans(data: Partial<Seans> & { patientId?: string }): void {
  ...
  getDb().prepare(
    `INSERT INTO seanslar (id, client_id, tarih, sure, konu, notlar, odev) VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(id, clientId, data.tarih, data.sure ?? 50, data.konu ?? null, data.notlar ?? null, data.odev ?? null);
}

export function deleteSeans(id: string): void {
...
export function getAllEvents(): CalendarEvent[] {
  return getDb().prepare('SELECT * FROM calendar_events ORDER BY start ASC').all() as CalendarEvent[];
}

export function createEvent(data: Partial<CalendarEvent>): void {
  const id = data.id ?? `${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
  getDb().prepare(
    `INSERT INTO calendar_events (id, title, start, end_time, notes) VALUES (?, ?, ?, ?, ?)`
  ).run(id, data.title, data.start, data.end_time ?? (data as any).end ?? null, data.notes ?? null);
}

export function updateEvent(id: string, data: Partial<CalendarEvent>): void {
  ...
  if (updates.length === 0) return;
  values.push(id);
  getDb().prepare(`UPDATE calendar_events SET ${updates.join(', ')} WHERE id = ?`).run(...values);
}
...
export function getAllPending(): PendingFile[] {
  return getDb().prepare('SELECT * FROM pending_files ORDER BY created_at DESC').all() as PendingFile[];
}

export function createPending(data: Partial<PendingFile> & { adSoyad?: string; randevuTarihi?: string; not?: string }): void {
  const id = data.id ?? `${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
  getDb().prepare(
    `INSERT INTO pending_files (id, ad_soyad, randevu_tarihi, not_text, status, drop_reason, dropped_at) VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
...
export function updatePending(id: string, data: Partial<PendingFile> & { dropReason?: string; droppedAt?: string }): void {
  const updates: string[] = [];
  const values: unknown[] = [];
  ...
  if (updates.length === 0) return;
  values.push(id);
  getDb().prepare(`UPDATE pending_files SET ${updates.join(', ')} WHERE id = ?`).run(...values);
}

export function deletePending(id: string): void {
  getDb().prepare('DELETE FROM pending_files WHERE id = ?').run(id);
}

// ─── SMS Log ─────────────────────────────────────────────────────────────────

export function getAllSms(): SmsLog[] {
  return getDb().prepare('SELECT * FROM sms_log ORDER BY created_at DESC LIMIT 200').all() as SmsLog[];
}

export function createSms(data: Partial<SmsLog> & { trigger?: string }): string {
  const id = data.id ?? `${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 7)}`;
  getDb().prepare(
    `INSERT INTO sms_log (id, phone, name, message, trigger_type, status, error, sent_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id, data.phone, data.name ?? null, data.message,
    data.trigger_type ?? (data as any).trigger ?? 'manual',
    data.status ?? 'queued', data.error ?? null, data.sent_at ?? null,
  );
  return id;
}

export function updateSms(id: string, data: Partial<SmsLog> & { delivery_status?: string; delivered_at?: string }): void {
  ...
  if (updates.length === 0) return;
  values.push(id);
  getDb().prepare(`UPDATE sms_log SET ${updates.join(', ')} WHERE id = ?`).run(...values);
}
